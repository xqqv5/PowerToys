    ...
    public sealed class BuiltinProvider : IDisposable
    {
        ...
        public void RegisterProvidersTo(ActionProviderRuntime runtime)
        {
            ...
            foreach (var provider in _providers)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    try
                    {
                        System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to register provider '{provider.Id}': {ex.Message}");
                    }
                    catch
                    {
        ...
        public async Task<List<ProfileGroup>> GetDefaultProfileGroupsAsync()
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to get workspace groups: {ex.Message}");
                }
                catch
                {
                    // Ignore logging errors
            ...
            catch (Exception ex)
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to get MCP provider groups: {ex.Message}");
                }
                catch
                {
                    // Ignore logging errors
        ...
        private void LoadWorkspaceProvider()
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to load workspace provider: {ex.Message}");
                }
                catch
                {
                    // Ignore logging errors
        ...
        private void LoadMcpProviders()
        {
            try
            {
                ...
                foreach (var config in configs)
                {
                    ...
                    catch (Exception ex)
                    {
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to create provider '{config.Id}': {ex.Message}");
                        }
                        catch
                        {
                            // Ignore logging errors
            ...
            catch (Exception ex)
            {
                try
                {
                    System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to load MCP providers: {ex.Message}");
                }
                catch
                {
                    // Ignore logging errors
        ...
        private async Task<List<ProfileGroup>> GetDynamicMcpProviderGroupsAsync()
        {
            ...
            try
            {
                ...
                foreach (var config in mcpConfigs)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        try
                        {
                            System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to create group for MCP provider '{config.Id}': {ex.Message}");
                        }
                        catch
                        {
                            // Ignore logging errors
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    System.Diagnostics.Debug.WriteLine($"BuiltinProvider: Failed to load dynamic MCP provider groups: {ex.Message}");
                }
                catch
                {
                    // Ignore logging errors
