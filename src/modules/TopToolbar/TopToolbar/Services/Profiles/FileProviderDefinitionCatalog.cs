...
public sealed class FileProviderDefinitionCatalog : IProviderDefinitionCatalog
{
    ...
    public FileProviderDefinitionCatalog(string providersDirectory = null)
    {
        _providersDirectory = string.IsNullOrWhiteSpace(providersDirectory)
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Microsoft", "PowerToys", "TopToolbar", "config", "providers")
            : providersDirectory;
    ...
    public IReadOnlyDictionary<string, ProviderDefinitionFile> LoadAll()
    {
        ...
        {
            Logger.LogWarning($"ProviderCatalog: enumerate failed - {ex.Message}.");
            return result;
        }
        ...
        foreach (var file in files)
        {
            try
            {
                ...
                {
                    Logger.LogWarning($"ProviderCatalog: duplicate providerId '{def.ProviderId}' in '{file}', ignoring.");
                    continue;
                }
            ...
            {
                Logger.LogWarning($"ProviderCatalog: failed to load '{file}' - {ex.Message}.");
            }
        }
